...
- **โทเค็นหมดอายุอัตโนมัติไม่ตัดสิทธิ์กลางเกมอีกต่อไป** — Supabase หมุน access token
  ...
  **ก่อน**ที่จะเรียกเซิร์ฟเวอร์ด้วยซ้ำ ถ้าเครือข่ายสะดุดตอนนั้นพอดี ผู้เล่นจะเสีย
  G-Signal ไปเรื่อย ๆ จนกว่าจะยืนยันสำเร็จครั้งถัดไป — ขัดกับกติกาที่ว่า G-Sentry/
  G-Signal ต้องทำงานต่อได้แม้ขาดการเชื่อมต่อ ตอนนี้: (1) ฝั่ง Rust จำผลยืนยันสิทธิ์
  ล่าสุดไว้ 24 ชม. ถ้าการยืนยันซ้ำเจอปัญหาเครือข่าย จะใช้สิทธิ์เดิมต่อ (ทำเครื่องหมาย
  `stale` ให้ผู้เล่นเห็นตรง ๆ) แทนที่จะตัดสิทธิ์ทันที — การยืนยันครั้งแรกตอนเปิดแอป
  (ไม่มีอะไรให้อ้างอิง) ยังคงต้องต่อเน็ตเหมือนเดิม และการถูกปฏิเสธสิทธิ์จริงจากเซิร์ฟเวอร์
  ก็ยังตัดสิทธิ์ทันทีเหมือนเดิม (2) ฝั่งหน้าจอ แยกการหมุนโทเค็นเฉย ๆ ออกจากการ
  ล็อกอิน/ล็อกเอาต์จริง — เดิมทั้งดาดฟ้าคำสั่ง (Command Deck) จะหายวับไปเป็นหน้าจอ
  "กำลังตรวจสอบสิทธิ์" เต็มจอทุกครั้งที่โทเค็นหมุน แม้จะสำเร็จเงียบ ๆ ในเสี้ยววินาทีก็ตาม
- **หน้า Live เลิกโชว์ "3 active alerts" ค้างตลอดเวลา** — ตัวเลขนี้เป็นค่าคงที่ที่ฝังไว้
  และ `buildMatch()` ไม่เคยเขียนทับ ทำให้หน้า Live ขึ้น "3 active alerts" **ตลอดเวลา
  รวมถึงตอนอยู่ในแมตช์จริง** ตอนนี้นับจากของจริง (ฮีโร่ที่หายจากแมพ + สัญญาณแก๊งค์)
  และขึ้น `—` เมื่อเซ็นเซอร์ไม่น่าเชื่อถือ — เพราะ "0 alerts" ก็เป็นการอ้างพอ ๆ กับ "3"
  ถ้าไม่มีอะไรมองอยู่เลย
- **แท็บ "บิลด์" แสดงข้อมูลจริงแล้ว** — เดิมไม่มีอะไรป้อนข้อมูลให้หน้านี้เลย มันจึงขึ้น
  `hero: "Maiden"` กับ `lane: "Support"` ซึ่งเป็นข้อความฝังตายทั้งคู่ ราวกับเป็นฮีโร่และเลน
  ของผู้เล่นจริง ๆ พร้อมการ์ดว่างเปล่าสองใบ ตอนนี้: ชื่อฮีโร่มาจาก tick จริง, ไอเทมมาจาก
  `item_names` ที่ GSI ส่งมาตลอดแต่ฝั่ง deck ไม่เคยประกาศ type ไว้, และคำแนะนำมาจาก
  G-Master จริง (`advice-update` ซึ่งเดิมไปถึงแค่ overlay กับหน้า Settings)
  ส่วนที่ดึงไม่ได้จริง ๆ ขึ้น `—` ไม่เดา: **เลน** (GSI ไม่บอกเลย) และ **ไอเทมชิ้นถัดไป**
  (G-Master ตอบเป็นประโยคภาษาไทย ไม่ใช่ข้อมูลมีโครงสร้าง)
- **ศัตรูที่ตายแล้วไม่นับเป็นภัยแก๊งค์อีกต่อไป** — ศัตรูที่ตายจะหายจากมินิแมพเหมือนกับหลบเข้า
  fog ทุกประการ G-Sentry จึงตีเป็น "หายจากแมพ" และ G-Motion นับเป็นภัยคุกคาม ผลคือ
  **ตอนที่ทีมเราชนะการปะทะและกวาดศัตรูครบ 5 ตัว — ซึ่งเป็นจังหวะที่แมพปลอดภัยที่สุด — ระบบ
  กลับเตือนแรงที่สุด (~90%)** ตอนนี้จำนวนศัตรูที่ตายอ่านจากสกอร์ทีมเรา (`map.*_score` คือ
  จำนวนคิลของทีมนั้น = จำนวนตายของอีกฝั่ง ซึ่งแม่นยำ) แล้วหักออกจากจำนวนภัยคุกคาม
  ข้อจำกัดที่ยอมรับโดยตั้งใจ: GSI ไม่บอกว่า *ตัวไหน* ตาย ระบบจึงหักแบบ "นับจำนวน" ไม่ระบุตัว
  และเลือกตัดฮีโร่ที่ความเสี่ยง **ต่ำสุด** ออกก่อน — เดาผิดแล้วยังเตือนไว้ ดีกว่าเดาผิดแล้วเงียบ
  ระยะเวลาที่หักใช้ตารางเวลาเกิดใหม่แบบ Turbo (สั้นกว่า) เป็นค่าประมาณ เพราะเลเวลของศัตรู
  ไม่สามารถมองเห็นได้ และการประเมิน "สั้นเกินไป" ปลอดภัยกว่า "ยาวเกินไป"
