...
- **Phase 2 ✅ ปิดครบ** (pushed ถึง `1b5c8a3`): (1) `3d7c1d0` AI เติม name_en ในหน้า save (list clip TH+EN input,
  ปุ่ม AI → llm_complete), (2) `64bf80e` backlink + multi-event chips ในคลัง (SoundRow: events chips + 🔗 → sound_usage),
  (3) `1b5c8a3` **impact-radar** (`ImpactRadar.tsx`: SVG radial voice(กลาง)→events(วงใน RE=88)→packs(วงนอก RP=150),
  เปิดจากปุ่ม Radar บน SoundRow, ใช้ sound_usage. geometry verified: 0 NaN/0 OOB). ทุกอย่าง tsc 0, frontend-only.
- **`809e717` bugfix**: (1) pack row draggable → ลากมา canvas ได้ (query_sounds→setClips), (2) Clip เพิ่ม `source`
  → library sound พก source_path เข้า clip → save_clips เก็บ per-clip source (pack อ้าง origin จริง), (3) file-clip
  save copy ไฟล์ตัวเอง (เดิมตัดจาก source wav เดียว = เสียงผิด). source_path อยู่ใน DB อยู่แล้ว แต่เดิม Clip ไม่พก.
- **เหลือ live-test (Boss)**: drag pack/sound มา canvas, save file-clip (source ต่อ clip), AI name-gen, backlink/radar.
...
- ⚠️ **`pack_mrijgajn` ถูกล็อกด้วย G-Maiden test** (`voice_api.rs::real_pack_mrijgajn_maps_voice_and_banners`,
