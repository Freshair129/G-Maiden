...
- **Symptom:** [`src-tauri/src/voice_api.rs`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs) joined three attacker-influenced
  ...
  the result ([`audio::play_file`](file:///g:/G-Maiden/src-tauri/src/audio.rs#L145), via [`active_event_clips`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L654) /
  [`resolve_existing_clips`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L807)) or reading it into a base64 `data:` URL and
  broadcasting it on the Tauri event bus to the overlay renderer
  ([`fired_banner`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L844) → `announcer-banner`, and the pack-inventory cover/clip
  tiles built by [`build_pack`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L1130), which runs on every [`state()`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L394) call — i.e.
  merely opening Audio Settings, not only on a fired event). Separately,
  [`voice_api::import_archive`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L510) shelled out to PowerShell `Expand-Archive` with
  no app-level validation of entry paths, so zip-slip safety depended
  entirely on the .NET implementation.
- **Evidence:**
  - [`resolve_existing_clips`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L807) (pre-fix): `clips.iter().map(|rel| dir.join(rel.replace('\\','/'))).filter(|p| p.is_file())`
  ...
  - [`fired_banner_from`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L857) (pre-fix): `dir.join(mapping.banner_asset.replace('\\','/'))` fed
    directly into [`read_banner_data_url`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L991), which reads up to 3MB and
    base64-inlines it — no existence/containment check before the read.
  - [`build_pack`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L1130)'s `manifest.cover_image` resolution had the identical
    unguarded join, and is reachable just by listing packs ([`state()`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L394)),
    confirmed by tracing [`voice_api::state()`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L394) → [`build_pack()`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L1130) →
    [`read_banner_data_url()`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L991) with no caller-side gate.
  ...
  - Full test suite added in this fix ([`src-tauri/src/voice_api.rs`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs) `mod
    ...
    nested path (no false positive), and three [`extract_pack_zip`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L565) tests
...
- **Root Cause:** the codebase had one existing, well-tested sanitizer
  ([`sanitize_id`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L1406), applied to `pack_id` and to *upload* file names via
  [`sanitize_file_name`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L1422)) but no equivalent guard for strings that arrive
...
- **Why it escaped detection:** `cargo test`/`cargo clippy` (the project's
  ...
  security-focused test for [`voice_api.rs`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs). The independent full-system audit
  ...
  reachable just by opening Audio Settings ([`build_pack`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L1130)/[`state()`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L394)), not only
  on a fired event — this RCA and fix directly close that finding.
- **Prevention:** one shared, unit-tested helper,
  [`voice_api::safe_pack_path(pack_dir: &Path, rel: &str) -> Option<PathBuf>`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L726),
  is now the single choke point for every manifest-derived path join in
  [`voice_api.rs`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs) ([`resolve_existing_clips`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L807)/[`active_event_clips`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L654),
  [`fired_banner_from`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L857), [`preview_clip`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L897), [`install_report`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L925), and [`build_pack`](file:///g:/G-Maiden/src-tauri/src/voice_api.rs#L1130)'s
