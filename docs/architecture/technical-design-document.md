...
> Source of truth ของ "จะสร้างยังไง". อ่านคู่กับ [[engineering-spec]] (contracts).
...
│  ├─ App.tsx            # overlay window + window routing; CommandDeck.tsx = control window
│  ├─ src/live/           # live-wire builders (Tauri events → UI state); src/gid.ts = GID codec
│  └─ (accounts/GID layer: identity.rs + Supabase gstore — see ADR-14, CR-002-Phase2)
