...
- โมเดลตระกูล **reasoning/thinking** (เช่น `qwen3.5:4b`) จะส่งเอาต์พุตทาง `message.thinking`
  **ไม่ใช่** `message.content`. ถ้า wrapper เก็บแค่ `content` จะได้ log ว่างทั้งที่ใช้ token ไปแล้ว.
- ที่แย่กว่า: ถ้า `num_predict` น้อย (เช่น 512) โมเดลใช้ budget หมดไปกับ thinking **ก่อนถึงคำตอบจริง**
  → content ว่างเปล่า.
- **แก้:** ส่ง `"think": false` ใน request `/api/chat` สำหรับงาน draft/config (เราต้องการคำตอบตรง
  ไม่ใช่ reasoning). พิสูจน์แล้ว: qwen3.5:4b + `think:false` → ออก content ทันที.

### อย่านับ "ว่าง" เป็น "สำเร็จ"
- เงื่อนไข done ต้องเช็คว่ามี **content จริง** ไม่ใช่แค่ event `done` มาถึง.
- ถ้า content ว่าง → mark **failed/EMPTY** + เขียนเหตุผลใน log (escalate, ไม่ใช่ done หลอก).
- เก็บทั้ง `thinking` และ `content` ลง log (คั่นด้วย marker) เพื่อ debug ได้.

### เลือกโมเดลตามงาน — โมเดลที่ตามโครง scaffold ได้ดีกว่า ชนะ
เทียบจริงบน task เดียวกัน (สร้าง GitHub Actions CI ของ Tauri, มี scaffold ให้):

| โมเดล | ตามโครง scaffold | name/on keys | thinking waste |
| --- | --- | --- | --- |
| `qwen3.5:4b` (fast) | บางส่วน | ตัดทิ้ง | สูง (ต้อง think:false) |
| `gemma4-rust-coder:latest` (balanced) | เต็ม | ครบ | ไม่มี |

- โมเดลที่ fine-tune มาตรงภาษาโปรเจกต์ (เช่น rust-coder สำหรับ repo Rust) **ทำตาม scaffold ครบกว่า**
  และคุณภาพโครงสร้างดีกว่าโมเดล general ขนาดใกล้กัน.
- `num_predict` ที่สูงขึ้น (profile `balanced`/`ui-heavy`) ช่วยให้ตอบครบ ไม่ถูกตัดกลางคัน.

### ย้ำ: ผลลัพธ์ local = draft
เอาต์พุตจากโมเดลเล็กเป็น **ร่างที่มีโครงครบ** ให้คนหรือ full agent (Claude) เกลา final —
มักมีจุดต้องแก้ (เช่น ชื่อ GitHub Action ที่ไม่มีจริง, ลืม prefix `pnpm`). เหมาะกับงาน config/schema/
docs/scaffold ที่ประหยัดโควต้า ไม่เหมาะกับงานที่ต้องลงมือแก้ repo/รันคำสั่งเอง.
