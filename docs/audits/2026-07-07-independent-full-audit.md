...
**Medium (<1 week):** Fix Dire blindness (#1); silent-CV watchdog + loud Lite-mode banner (#5); gate the release workflow (#4); lift onboarding/updater/settings-sync to app root (#11); collapse [`useCompanionData`](file:///g:/G-Maiden/src/src/companion.ts#L941) to one provider (#11/H1); wire enemy context + [`counter_advice`](file:///g:/G-Maiden/src-tauri/src/counter_advice.rs) into G-Master (#6 partial); swap offline model (#16); overlay HUD no-go zones (#O1); consolidate color tokens + purge dead CSS (#20); error boundaries; move TTS off the capture thread (#13).
