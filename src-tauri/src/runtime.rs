...
/// Wall-clock ms. Used by the dead-enemy pool above and the entitlement grace
/// cache further down. `gsi.rs` and `utterance.rs` each carry their own
/// private copy of this same helper — module-private by design, so no shared
/// `pub(crate)` version to reuse across files.
fn epoch_ms() -> u64 {
    SystemTime::now()
...
mod tests {
    ...
    /// Same reasoning as [`match_state_guard`], for the entitlement grace
    /// cache: a separate process-global static, so it needs its own lock
    /// rather than sharing one that has nothing to do with match state.
