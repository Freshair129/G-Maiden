# gen-dataset — synthetic minimap hero-detector dataset (G-Maiden P2.1)

Generates labeled `patch_size × patch_size` PNG patches for training an ONNX
...
Spike S-1 (`docs/SPIKE--S-1-MINIMAP-CV.md`) proved the minimap CV hot-loop
...
the model at runtime: `icon×icon` candidate crops, resized to the model input.

## Degradation profile (matches the spike exactly)

| Augmentation | Value | Spike source |
| --- | --- | --- |
| Fog/visibility dimming | `rgb *= 0.55 + 0.45*rand` (0.55–1.0) | `dim` |
| Sub-pixel jitter | gaussian σ = 0.6 px on x,y (rounded to int shift) | `jitter_x/y` |
| Additive per-channel noise | gaussian σ = 0.035 on 0..1, then clamp | inner loop |
| Partial occlusion | 18% chance, right half hidden | `occlude` |
| Distractors (negatives) | 2–3 px team-colored dots (red/green) | step 2 |
...
Zero-asset smoke run (synthetic icons + procedural backgrounds — always works):
...
| `--seed` | `20260621` | RNG seed; same seed → identical dataset. |
...
val   = ImageFolder("dataset/val")   # synthetic — see domain-gap note below
```
...
**Hero minimap icons** — the canonical source is Valve's game files:
`game/dota/pak01_dir.vpk` → `panorama/images/heroes/icons/` (the *minimap*
...
serves the **portrait** icon, *not* the minimap blip — prefer the VPK minimap
assets for fidelity. **This tool intentionally does not download anything**; drop
files in yourself.

**Backgrounds** — crop the minimap square (bottom-left of the HUD, ≈15.6% of
...
- **Synthetic ≠ real.** Reserve **real gameplay footage for VALIDATION ONLY** —
  never train on it — so the held-out metric honestly measures the synthetic→real
...
determinism (same seed → identical hashes), patch dims == `--patch-size`,
