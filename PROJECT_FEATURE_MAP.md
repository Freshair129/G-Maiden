...
| In-app updater | [`useAppUpdate.ts`](file:///g:/G-Maiden/src/src/useAppUpdate.ts) | 🔴 **BROKEN** — the UI (launch auto-check, banner, manual check, install/relaunch) is written and minisign verification is real, but no manifest resolves, so it can never surface an update. Bare `check()` requests `release/channels/windows.json` (404, swallowed by an empty `catch`) and never consults `updateChannel.ts`; the channel-aware caller in `GmadFirstRunGate.tsx` reaches the manifest but looks up a `platforms` key it does not carry. See CLAUDE.md → Release & update workflow |
