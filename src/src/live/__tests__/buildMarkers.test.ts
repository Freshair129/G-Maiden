...
describe("buildMarkers", () => {
  it("returns base unchanged when cv is null and missingPos is empty", () => {
    const result = buildMarkers(null, new Map(), MOCK.markers);
    expect(result).toBe(MOCK.markers);
  });

  it("returns base unchanged when region.side is 0 and missingPos is empty", () => {
    const cv = makeCv({ region: { x: 0, y: 0, side: 0 } });
    const result = buildMarkers(cv, new Map(), MOCK.markers);
    expect(result).toBe(MOCK.markers);
  });

  it("returns base unchanged when detections array is empty and missingPos is empty", () => {
    const cv = makeCv({ detections: [] });
    const result = buildMarkers(cv, new Map(), MOCK.markers);
    expect(result).toBe(MOCK.markers);
  });

  it("returns base unchanged when cv is empty and missingPos is empty", () => {
    const result = buildMarkers(null, new Map(), MOCK.markers);
    expect(result).toBe(MOCK.markers);
  });
  ...
  it("adds a missing-enemy marker from missingPos at normalised [0.6, 0.4] -> x=60, y=40", () => {
    const missingPos = new Map<string, [number, number]>([
      ["npc_dota_hero_crystal_maiden", [0.6, 0.4]]
    ]);
    const result = buildMarkers(null, missingPos, MOCK.markers);
  ...
  it("combines cv detections and missing-enemy markers together", () => {
    const cv = makeCv({
      detections: [makeDetection({ name: "npc_dota_hero_crystal_maiden", x: 100, y: 100 })]
    });
    const missingPos = new Map<string, [number, number]>([
      ["npc_dota_hero_warden", [0.1, 0.9]]
    ]);
    const result = buildMarkers(cv, missingPos, MOCK.markers);
    expect(result).toHaveLength(2);
    expect(result[0].kind).toBe("hero");
    expect(result[1]).toMatchObject({ id: "miss-0", kind: "enemy", state: "missing", x: 10, y: 90 });
  });

  it("skips a missing-enemy marker whose position is NaN", () => {
    const missingPos = new Map<string, [number, number]>([
      ["npc_dota_hero_warden", [NaN, 0.5]]
    ]);
    const result = buildMarkers(null, missingPos, MOCK.markers);
